...
class PokemonService {
    ...
    async createPokemons(pokemonsData: PokemonType[]){
        if (pokemonsData.length == 0){
            return ErrorMessagePokemons.LISTA_POKEMONS_NULL
        }
        else {
            this.getPokemons()
            this.mapPokemons()
    ...
    async getPokemonsByType(type: String){
        ...
        if(pokemons.length  === 0){
            return ErrorMessagePokemons.TYPE_NOT_EXIST
        }
        else{
            return pokemons
        }
    ...
    async getPokemonByDex(dex: number){
        ...
        if(pokemon === null){
            return ErrorMessagePokemons.DEX_NOT_EXIST
        }
        else{
            return pokemon         
        }
    ...
    async getPokemonByName(name: string){
        ...
        if(pokemon === null){
            return ErrorMessagePokemons.NAME_NOT_EXIST
        }
        else{
            return pokemon         
        }
